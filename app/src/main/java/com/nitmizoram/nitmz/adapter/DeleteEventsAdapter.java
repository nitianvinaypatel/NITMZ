...
import com.nitmizoram.nitmz.UpdateEvents;
...
public class DeleteEventsAdapter extends RecyclerView.Adapter<DeleteEventsAdapter.DeleteStaffViewHolder> {

    DatabaseReference reference;
    private Context context;
    private ArrayList<FacultyData> list;

    public DeleteEventsAdapter(Context context, ArrayList<FacultyData> list) {
    ...
    public void onBindViewHolder(@NonNull DeleteStaffViewHolder holder, int position) {
        ...
        holder.delete.setOnClickListener(view -> {
            ...
            reference.child("Events").child(facultyData.getKey()).removeValue().addOnCompleteListener(new OnCompleteListener<Void>() {
                @Override
                public void onComplete(@NonNull Task<Void> task) {
                    Toast.makeText(context,"Events details deleted successfully",Toast.LENGTH_SHORT).show();
                }
            }).addOnFailureListener(e -> Toast.makeText(context,"Failed to delete events details",Toast.LENGTH_SHORT).show());
        });
